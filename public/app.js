    ...
    initAccount: async () => {
        if (localStorage.getItem('account') == null || localStorage.getItem('account') == "") {
            ...
            httpRequest('/publickey', { public: this.account.address, token: localStorage.getItem("token") }, (data, textStatus, jQxhr) => {
                if (data.status != "OK") {
                    alert("You have already voted so please refer to the voted reciept.")
                    localStorage.clear()
                    window.location = "/"
                }
            })
    ...
    eventBinder: async () => {
        $(".vote-btn").click(async (e) => {
            ...
            try {
                ...
                alert("You successfully casted your vote. Please download the reciept you get now for future reference.")
